...
#         emmbedding = face.embedding.tolist()
        ...
        if (await FaceService.registrar_rostro(request, request.app)):
    ...
    try:
        ...
        if pwd_context.verify(data.get("password"), usuario.password_hash):
            ...
            if  usuario.rol == "ESTUDIANTE":
                estudiante : Estudiante = await EstudianteService.obtener_estudiante_by_usuario(usuario.id)
                print("Estudiante ", estudiante)
            
            data_usuario = {"id": usuario.id, "nombre": usuario.nombre, "apellido": usuario.apellido, "email": usuario.email,"foto_perfil": usuario.foto_perfil, "rol": usuario.rol}
           
            data : EstudianteCreate = EstudianteCreate(usuario=data_usuario, estudiante=estudiante)

            return {"result":"ok", "message":"Usuario autenticado","data": data}
        else:
